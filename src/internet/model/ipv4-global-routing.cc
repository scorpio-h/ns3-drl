  ...
  static TypeId tid = TypeId ("ns3::Ipv4GlobalRouting")
    ...
    .AddAttribute("PerflowEcmpRouting",
                  "Set to true if flow are randomly routed among ECMP",
                   ...
                   MakeBooleanChecker ())
    .AddAttribute("PerflowWcmpRouting",
                  "Set to true if flow are randomly routed among WCMP",
                   BooleanValue (false),
                   MakeBooleanAccessor (&Ipv4GlobalRouting::m_perFlowWcmpRouting),
                   MakeBooleanChecker ())
    .AddAttribute("NewWeight",
                  "Set to true if weight.txt has been read",
                   BooleanValue (false),
                   MakeBooleanAccessor (&Ipv4GlobalRouting::m_read),
                   MakeBooleanChecker ())
    .AddAttribute("WeightFile",
                  "position of weight.txt",
                   StringValue ("scratch/rl-wcmp/Weight.txt"),
                   MakeStringAccessor (&Ipv4GlobalRouting::m_filename),
                   MakeStringChecker ())
...
Ipv4GlobalRouting::LookupGlobal (Ipv4Address dest, Ptr<Packet> packet, const Ipv4Header &header, uint32_t flowId, Ptr<NetDevice> oif)
{
  ...
  if (allRoutes.size () > 0 ) // if route(s) is found
  {
      ...
      else if (m_perFlowWcmpRouting && flowId != 0) // If the flow id is 0, it may be the socket setup endpoint request, we simply return the first
      {                                           // available route to indicate the address is not local
          if(allRoutes.size()==8){
              bool exist = false;
              int id = m_ipv4->GetObject<Node> ()->GetId ()-8;
              for (std::size_t i = 0; i <m_flows.size(); i++)
              {
	          if(flowId == m_flows[i].flowid && id == m_flows[i].id){
                         selectIndex=m_flows[i].port;
                         exist = true;
                         break;
                   }
              }
...
/*
                  if(!m_read){
                      ...
                      std::ifstream t("scratch/rl-wcmp/Weight.txt"); 
                      std::stringstream buffer;  
                      buffer << t.rdbuf();  
                      for(int i=0;i<8;i++){
                          for(int j=0;j<8;j++){
                              for(int k=0;k<8;k++){
                                  buffer>>m_array[i][j][k];
                                  //std::cout<<m_array[i][j][k]<<" ";
                              }
                          }
                      }
                      m_read = true;
...
*/
                  ...
                  int destid = 0;
                  for(int i=0;i<32;i++){
                      if(dest == m_hostleaf[i].host){
                           destid = m_hostleaf[i].leaf;
                           break;
                      }
                  }   
                  if(b<=m_array[id][destid][0])
                        selectIndex = 0;
                  else{
                        for(int i=1;i<8;i++){
                            if(b<=m_array[id][destid][i] && b>m_array[id][destid][i-1]){
                                selectIndex = i;
                                break;
                            }
                         }
         ...
         //std::cout<<"Per flow WCMP is enabled, select index: " << selectIndex << " for flow: " << flowId<<std::endl;
         NS_LOG_LOGIC ("Per flow WCMP is enabled, select index: " << selectIndex << " for flow: " << flowId);
      }
      else
      {
          selectIndex = 0;
...
Ipv4GlobalRouting::RouteOutput (Ptr<Packet> p, const Ipv4Header &header, Ptr<NetDevice> oif, Socket::SocketErrno &sockerr)
{
  ...
  if(m_perFlowWcmpRouting && p != NULL) 
  {
    NS_ASSERT(m_randomEcmpRouting == false);
    FlowIdTag flowIdTag;
    bool found = p->PeekPacketTag(flowIdTag);
