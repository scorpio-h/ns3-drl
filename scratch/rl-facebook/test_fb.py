...
MAX_STEPS = 1000
stepTime = 0.001 # seconds =1ms
simTime = stepTime*MAX_STEPS+0.000001 # seconds
seed = 12
...
try:
    while True:
        ...
        while True:
            ...
            vector_to_file(a_t[0], folder + 'rewardLog.csv', 'a')
            vector_to_file(r_t, folder + 'actionLog.csv', 'a')
            vector_to_file(noise_t[0], folder + 'noiseLog.csv', 'a')
        ...
        if np.mod((currIt+1), 2) == 0:   # writes at every 2nd episode
            if (train_indicator):
                actor.model.save_weights(folder + "actormodel.h5", overwrite=True)
                actor.model.save_weights(folder + "actormodel" + str(stepIdx) + ".h5")
                with open(folder + "actormodel.json", "w") as outfile:
