...
Ptr<OpenGymDataContainer> MyGetObservation(void)
{
  ...
  for(int i=0;i<50;i++){
      box->AddValue(activelist[i]);
  }
  for(int i=0;i<700;i++){
      box->AddValue(finishedlist[i]);
  }
  return box;
}
...
void ScheduleNextStateRead(double envStepTime, FlowMonitorHelper* fmhelper, Ptr<FlowMonitor> flowMon,Ipv4GlobalRoutingHelper globalRoutingHelper,Ptr<OpenGymInterface> openGym)
{
    activelist.clear();
    finishedlist.clear();
    ...
    {	
        ...
        if(!(stats->second.txBytes == 52 * stats->second.txPackets + 4)){
            std::map<Ipv4Address, int>::iterator iter; 
            if(stats->second.rxPackets == stats->second.txPackets || stats->second.rxPackets + stats->second.lostPackets == stats->second.txPackets){                                               
                 ...
                 finishedlist.push_back(iter->second);
                 iter = hostid.find(fiveTuple.destinationAddress); 
                 finishedlist.push_back(iter->second);
                 finishedlist.push_back(fiveTuple.sourcePort);
                 finishedlist.push_back(fiveTuple.destinationPort);
                 finishedlist.push_back(0.0);
                 finishedlist.push_back(stats->second.rxBytes);
                 finishedlist.push_back(stats->second.timeLastRxPacket.GetSeconds()-stats->second.timeFirstTxPacket.GetSeconds());
             }
             else{
                 iter = hostid.find(fiveTuple.sourceAddress);  
                 activelist.push_back(iter->second);
                 iter = hostid.find(fiveTuple.destinationAddress); 
                 activelist.push_back(iter->second);
                 activelist.push_back(fiveTuple.sourcePort);
                 activelist.push_back(fiveTuple.destinationPort);
                 activelist.push_back(0.0);
            }
        }
    }
    activelist.resize(50,0.0);
    if(finishedlist.size()>700){
        int diff = finishedlist.size()-700;
        finishedlist.erase(finishedlist.begin(),finishedlist.begin()+diff);
    }
    else{
        finishedlist.resize(700,0.0);
    }
    th1 = th2;
    th2 = th;
    Simulator::Schedule (Seconds(envStepTime), &ScheduleNextStateRead, envStepTime, fmhelper, flowMon,globalRoutingHelper,openGym);
...
int main (int argc, char *argv[])
{
    ...
    uint32_t simSeed = 1;
    double envStepTime = 0.001; //seconds, ns3gym env step time interval 0.1ms
    uint32_t openGymPort = 5557;
    // Command line parameters parsing
    std::string id = "5";
    unsigned randomSeed = 0;
    std::string cdfFileName = "cdf/FB_CDF.txt";   //""
    double load = 0.9;   //0.0
    std::string transportProt = "Tcp";
    std::string weightFileName = "scratch/rl-facebook/Weight.txt";

    // The simulation starting and ending time
    double START_TIME = 0.0;
    double END_TIME = 1.00000001;  //5000steps
    ...
    CommandLine cmd;
    cmd.AddValue ("openGymPort", "Port number for OpenGym env. Default: 5557", openGymPort);
...
/*	        if (transportProt.compare ("DcTcp") == 0)
    ...
    Simulator::Schedule (Seconds(envStepTime), &ScheduleNextStateRead, envStepTime, &flowHelper,flowMonitor,globalRoutingHelper,openGym);
